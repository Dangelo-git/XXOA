...
//		mDataList = datas;
		...
		Log.i(TAG,"datas.size:"+datas.size()+" mCurrentSelected:"+mCurrentSelected+"mDataList.size:"+mDataList.size());
		invalidate();
	}
...
//			}
			...
			Log.i(TAG,"mDataList.size:"+mDataList.size()+" mCurrentSelected:"+mCurrentSelected);
			if(mDataList.size()==0){
				return;
			}
			canvas.drawText(mDataList.get(mCurrentSelected), x, baseline, mPaint);
