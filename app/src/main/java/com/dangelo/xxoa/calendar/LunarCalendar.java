...
public final class LunarCalendar {
	...
	private static String[] dayStrArray = { "", "一", "二", "三", "四", "五", "六",
			"七", "八", "九", "十", "十一", "十二" };
	private static String[] monthStrArray = { "", "正月", "二月", "三月", "四月", "五月", "六月",
		"七月", "八月", "九月", "十月", "十一月", "腊月" };
	...
	public static String getLauarFull(Calendar cld) {
	...
	public static String getLauarTail(Calendar cld) {
		int year = cld.get(Calendar.YEAR);
		int month = cld.get(Calendar.MONTH) + 1;
		int day = cld.get(Calendar.DAY_OF_MONTH);

		long[] lauarStrArray = calElement(year, month, day);
		String str = getFestival((int) lauarStrArray[1], (int) lauarStrArray[2]);
		if (str != null)
			return str;
		else
			return monthStrArray[(int) (lauarStrArray[1])]+getchina((int) (lauarStrArray[2]));
	}
