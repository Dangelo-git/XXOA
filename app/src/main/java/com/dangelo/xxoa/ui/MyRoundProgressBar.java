...
public class MyRoundProgressBar {
    ...
    private String TAG = "MyRoundProgressBar";
    ...
    public void initDialog() {
        if (dialog != null) {
            ...
            // lp.x = 100; // 新位置X坐标
            // lp.y = 100; // 新位置Y坐标
            lp.width = (int) (dm.widthPixels * 0.9);// DensityUtil.dip2px(context,
            // 290); // 宽度
    ...
    public boolean setProgress(int perValue, final long duration, int toValue) {
        ...
        if (perValue <= 0 || duration <= 0 || toValue <= 0
                || toValue < currentProsess || perValue > maxValue
                || toValue > maxValue) {
            return false;
        }
        mPerValue = perValue;
...
//		Log.i("MyRoundProgressBar", "dialog:" + dialog + "isShowing:"
//				+ isShowing());
