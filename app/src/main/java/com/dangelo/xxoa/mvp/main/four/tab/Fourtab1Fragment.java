...
import com.dangelo.xxoa.adapter.InnerMeetRecyclerAdapter;
import com.dangelo.xxoa.adapter.InnerMeetRecyclerAdapter;
...
public class Fourtab1Fragment extends BaseFragment implements FtabContact.View {

    private FtabPresenter ftabPresenter;
    private String TAG = Fourtab1Fragment.class.getSimpleName();
    private RecyclerView recyclerView;

    private RecyclerView listView;
    private List<InnerMeet> datas = new ArrayList<InnerMeet>();
    private InnerMeetRecyclerAdapter mAdAdapter;
...
//    public final static int TYPE_OUT=2;//未完成
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        ftabPresenter.setType(TYPE_INNER);
    ...
    public void refreshData(List<InnerMeet> data) {
        ...
        for (int i = 0; i < data.size(); i++) {
            JLog.i(TAG, "data.get(i).getAd_id():" + data.get(i).getConferenceName());
            if (TextUtils.isEmpty(data.get(i).getConferenceName())) {
            } else {
...
//            }
    ...
    private void setAdapter(final List<InnerMeet> data) {
//        JLog.i(TAG,"ShortCut.isShowAdList"+ShortCut.isShowAdList);
        mAdAdapter = new InnerMeetRecyclerAdapter(getActivity(), data, false,false);
        ...
        //设置加载更多触发的事件监听
        mAdAdapter.setOnLoadMoreListener(new InnerMeetRecyclerAdapter.OnLoadMoreListener() {
            @Override
        ...
        mAdAdapter.setOnItemClickListener(new InnerMeetRecyclerAdapter.MyItemClickListener() {
            @Override
            public void onItemClick(View view, int postion) {
                JLog.i(TAG,"onItemClick positon :"+postion);
                Bundle bundle=new Bundle();
                bundle.putSerializable("InnerMeet",data.get(postion));
                MeetDetailActivity.toThisActivity(getContext(),bundle);
            }
        });
        mAdAdapter.setOnItemDeleteClickListener(new InnerMeetRecyclerAdapter.MyItemDeleteClickListener() {
            @Override
